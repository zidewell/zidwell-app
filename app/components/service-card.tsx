...
export default function ServicesGrid({ onActionComplete, usage }: ServicesGridProps) {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
      {services.map((service) => {
        ...
        const hasAccess = isUtility ? true : canAccessFeature(service.featureKey);
        ...
        const hasBookkeepingTrial = service.featureKey === 'bookkeeping_access' && 
                                    usage?.bookkeepingTrial?.isActive;

        // Determine if button should be disabled
        const isDisabled = (requiresBVN && !isVerified) || (!hasAccess && !isUtility && !hasBookkeepingTrial);
        ...
        return (
          ...
          >
            ...
            {!hasAccess && !isUtility && !hasBookkeepingTrial && !requiresBVN && (
              <div className="absolute top-2 right-2">
            ...
            {userTier === 'free' && remaining !== null && !isUtility && !hasBookkeepingTrial && (
              <div className="absolute top-2 left-2">
                <span className={`text-xs font-medium px-2 py-1 rounded-full ${
                  remaining <= 1 ? 'bg-red-100 text-red-600' : 
                  remaining <= 3 ? 'bg-yellow-100 text-yellow-600' : 
            ...
            <h3 className="font-bold text-left text-gray-900 dark:text-gray-50">
              ...
              {!hasAccess && !isUtility && !hasBookkeepingTrial && !requiresBVN && (
                <span className="block text-xs text-[#C29307] mt-1">
            ...
            {userTier === 'free' && progress > 0 && !isUtility && !hasBookkeepingTrial && (
