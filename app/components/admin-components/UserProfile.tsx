      ...
      <AdminLayout>
        <div className="p-6">
          <div className="flex items-center gap-4 mb-6">
            <Button variant="outline" onClick={onBack} className="border-(--border-color) text-(--text-primary) hover:bg-[var(--bg-secondary)] squircle-md">
          ...
          <PaginationContent className="flex-wrap gap-1">
            <PaginationItem>
              <PaginationPrevious
                ...
                className={!pagination.hasPrevPage ? "pointer-events-none opacity-50" : "cursor-pointer text-(--text-primary) hover:bg-[var(--bg-secondary)] squircle-md"}
              />
            </PaginationItem>
            ...
            <PaginationItem>
              <PaginationNext
                onClick={() => handlePageChange(activeTab, pagination.currentPage + 1)}
                className={!pagination.hasNextPage ? "pointer-events-none opacity-50" : "cursor-pointer text-(--text-primary) hover:bg-[var(--bg-secondary)] squircle-md"}
              />
            </PaginationItem>
          </PaginationContent>
        </Pagination>
    ...
    switch (activeTab) {
      case "contracts":
        return currentItems.map((contract: any) => (
          <div key={contract.id} className="border border-(--border-color) squircle-lg p-4 hover:shadow-soft transition-shadow">
            <div className="flex justify-between items-start">
              <div>
                <h4 className="font-medium text-(--text-primary)">{contract.title || "Untitled Contract"}</h4>
                <p className="text-sm text-(--text-secondary)">ID: {contract.id}</p>
                <p className="text-sm">Status: {renderItemStatusBadge(contract.status)}</p>
                <p className="text-sm text-(--text-secondary)">Role: {contract.role}</p>
              </div>
              <div className="text-right">
                <p className="text-sm text-(--text-secondary)">{formatDate(contract.created_at)}</p>
                {contract.signed_at && (
      ...
      case "receipts":
        return currentItems.map((receipt: any) => (
          <div key={receipt.id} className="border border-(--border-color) squircle-lg p-4 hover:shadow-soft transition-shadow">
            <div className="flex justify-between items-start">
              <div>
                <h4 className="font-medium text-(--text-primary)">Receipt #{receipt.receipt_number}</h4>
                <p className="text-sm text-(--text-secondary)">ID: {receipt.id}</p>
      ...
      case "invoices":
        return currentItems.map((invoice: any) => (
          <div key={invoice.id} className="border border-(--border-color) squircle-lg p-4 hover:shadow-soft transition-shadow">
            <div className="flex justify-between items-start">
              <div>
                <h4 className="font-medium text-(--text-primary)">Invoice #{invoice.invoice_number}</h4>
                <p className="text-sm text-(--text-secondary)">ID: {invoice.id}</p>
              ...
              <div className="text-right">
                <p className="text-sm text-(--text-secondary)">{formatDate(invoice.created_at)}</p>
                <p className="font-medium text-(--text-primary)">{formatCurrency(invoice.total_amount)}</p>
              </div>
            </div>
          </div>
        ));
      ...
      case "transactions":
        return currentItems.map((transaction: any) => (
          <div key={transaction.id} className="border border-(--border-color) squircle-lg p-4 hover:shadow-soft transition-shadow">
            <div className="flex justify-between items-start">
              <div>
              ...
              <div className="text-right">
                <p className="text-sm text-(--text-secondary)">{formatDate(transaction.created_at)}</p>
                <p className={`font-medium ${transaction.type === "credit" ? "text-[var(--color-lemon-green)]" : "text-destructive"}`}>
                  {transaction.type === "credit" ? "+" : "-"}{formatCurrency(transaction.amount)}
                </p>
                <p className="text-sm text-(--text-secondary)">Balance: {formatCurrency(transaction.balance_after)}</p>
              </div>
            </div>
          </div>
        ));
      ...
      <>
        <div className="mb-6">
          <Input
            ...
            className="max-w-md border-(--border-color) bg-(--bg-primary) text-(--text-primary) focus:ring-(--color-accent-yellow) focus:ring-(--color-accent-yellow) squircle-md"
      ...
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-semibold font-(--font-space-grotesk) text-(--text-primary)">User Profile</h2>
              <p className="text-(--text-secondary)">Detailed view of user account and activities</p>
            </div>
          </div>
        ...
        <Tabs value={activeTab} onValueChange={handleTabChange} className="space-y-6">
          <TabsList className="grid w-full grid-cols-5 bg-[var(--bg-secondary)] p-1 squircle-md overflow-x-auto">
            <TabsTrigger 
              value="overview" 
              className="text-xs md:text-sm data-[state=active]:bg-[var(--color-accent-yellow)] data-[state=active]:text-[var(--color-ink)] text-(--text-secondary) squircle-sm transition-all whitespace-nowrap"
            >
              Overview
            </TabsTrigger>
            <TabsTrigger 
          ...
          <TabsContent value="overview" className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Card className="bg-(--bg-primary) border border-(--border-color) shadow-soft squircle-lg">
                <CardHeader>
                  <CardTitle className="font-(--font-space-grotesk) text-(--text-primary)">Basic Information</CardTitle>
                  <CardDescription className="text-(--text-secondary)">User account details</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <p className="font-medium text-(--text-primary)">{user.phone || "Not provided"}</p>
                    </div>
                    <div>
                      <label className="text-sm font-medium text-(--text-secondary)">Wallet Balance</label>
              ...
              <Card className="bg-(--bg-primary) border border-(--border-color) shadow-soft squircle-lg">
                <CardHeader>
                  <CardTitle className="font-(--font-space-grotesk) text-(--text-primary)">Account Status</CardTitle>
                  <CardDescription className="text-(--text-secondary)">Login and account information</CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-1 gap-4">
                    ...
                    <div>
                      ...
                      <p className="font-medium text-(--text-primary)">{formatDate(user.last_login)}</p>
                    </div>
                    <div>
                      <label className="text-sm font-medium text-(--text-secondary)">Last Logout</label>
                      <p className="font-medium text-(--text-primary)">{formatDate(user.last_logout)}</p>
                    </div>
                    {user.is_blocked && (
                      <div>
                        <label className="text-sm font-medium text-(--text-secondary)">Block Reason</label>
                        <p className="font-medium text-destructive">{user.block_reason || "No reason provided"}</p>
                      </div>
                    )}
                  </div>
                </CardContent>
              ...
              <Card className="bg-(--bg-primary) border border-(--border-color) shadow-soft squircle-lg md:col-span-2">
                <CardHeader>
                  <CardTitle className="font-(--font-space-grotesk) text-(--text-primary)">Activity Summary</CardTitle>
                  <CardDescription className="text-(--text-secondary)">User engagement metrics</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                    <div className="text-center p-4 bg-[var(--bg-secondary)] squircle-lg">
                      <p className="text-2xl font-bold text-[var(--color-accent-yellow)] font-(--font-space-grotesk)">{totalCounts.contracts}</p>
                      <p className="text-sm text-(--text-secondary)">Contracts</p>
                    </div>
                    <div className="text-center p-4 bg-[var(--bg-secondary)] squircle-lg">
                      <p className="text-2xl font-bold text-[var(--color-lemon-green)] font-(--font-space-grotesk)">{totalCounts.receipts}</p>
                      <p className="text-sm text-(--text-secondary)">Receipts</p>
                    </div>
                    <div className="text-center p-4 bg-[var(--bg-secondary)] squircle-lg">
                      <p className="text-2xl font-bold text-purple-600 font-(--font-space-grotesk)">{totalCounts.invoices}</p>
                      <p className="text-sm text-(--text-secondary)">Invoices</p>
                    </div>
                    <div className="text-center p-4 bg-[var(--bg-secondary)] squircle-lg">
                      <p className="text-2xl font-bold text-orange-500 font-(--font-space-grotesk)">{totalCounts.transactions}</p>
          ...
          <TabsContent value="contracts">
            <Card className="bg-(--bg-primary) border border-(--border-color) shadow-soft squircle-lg">
              <CardHeader>
                <CardTitle className="font-(--font-space-grotesk) text-(--text-primary)">User Contracts</CardTitle>
                <CardDescription className="text-(--text-secondary)">All contracts created by this user</CardDescription>
              </CardHeader>
          ...
          <TabsContent value="receipts">
            <Card className="bg-(--bg-primary) border border-(--border-color) shadow-soft squircle-lg">
              <CardHeader>
                <CardTitle className="font-(--font-space-grotesk) text-(--text-primary)">User Receipts</CardTitle>
                <CardDescription className="text-(--text-secondary)">All receipts associated with this user</CardDescription>
              </CardHeader>
              <CardContent>{renderSearchAndResults()}</CardContent>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="invoices">
            <Card className="bg-(--bg-primary) border border-(--border-color) shadow-soft squircle-lg">
              <CardHeader>
                <CardTitle className="font-(--font-space-grotesk) text-(--text-primary)">User Invoices</CardTitle>
                <CardDescription className="text-(--text-secondary)">All invoices created for this user</CardDescription>
              </CardHeader>
              <CardContent>{renderSearchAndResults()}</CardContent>
            </Card>
          </TabsContent>
          ...
          <TabsContent value="transactions">
            <Card className="bg-(--bg-primary) border border-(--border-color) shadow-soft squircle-lg">
              <CardHeader>
                <CardTitle className="font-(--font-space-grotesk) text-(--text-primary)">User Transactions</CardTitle>
                <CardDescription className="text-(--text-secondary)">All financial transactions by this user</CardDescription>
              </CardHeader>
              <CardContent>{renderSearchAndResults()}</CardContent>
            </Card>
          </TabsContent>
