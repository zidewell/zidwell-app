  ...
  return (
    <div className="squircle-lg bg-(--bg-primary) border border-(--border-color) shadow-float p-5 sm:p-8">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          <p className="text-xs text-(--text-secondary)">Net balance · August</p>
          <p className="font-display text-3xl sm:text-4xl font-semibold text-(--text-primary)">₦ 4,820,300</p>
        </div>
        <div className="flex gap-2 text-[11px]">
          ...
          <span className="rounded-full bg-[var(--color-ink)]/5 dark:bg-white/10 px-3 py-1 font-medium text-(--text-secondary)">Expense ₦ 3.4M</span>
        </div>
      </div>
      <div className="mt-6 grid gap-6 md:grid-cols-3">
        <div className="md:col-span-2 squircle-sm bg-[var(--bg-secondary)] p-4 sm:p-5">
          <div className="flex items-center justify-between">
            ...
            <p className="text-[11px] text-(--text-secondary)">Last 8 months</p>
          </div>
          <div className="mt-5 flex items-end gap-2 sm:gap-4 h-40">
            {months.map((mo) => (
              <div key={mo.month} className="flex-1 flex flex-col items-center gap-1">
                ...
                <span className="text-[10px] text-(--text-secondary)">{mo.month}</span>
              </div>
            ))}
          </div>
        </div>
        <div className="squircle-sm bg-[var(--bg-secondary)] p-4 sm:p-5">
          <p className="text-sm font-medium text-(--text-primary)">Spending breakdown</p>
      ...
      <div className="mt-6 grid gap-3 sm:grid-cols-3">
        <div className="squircle-sm bg-[var(--bg-secondary)] p-4">
          <p className="text-[11px] text-(--text-secondary)">Monthly trend</p>
          <p className="font-display text-xl font-semibold text-[var(--color-lemon-green)]">+18.2%</p>
        </div>
        <div className="squircle-sm bg-[var(--bg-secondary)] p-4">
          <p className="text-[11px] text-(--text-secondary)">Largest category</p>
          <p className="font-display text-xl font-semibold text-(--text-primary)">Logistics</p>
        </div>
        <div className="squircle-sm bg-[var(--bg-secondary)] p-4">
          <p className="text-[11px] text-(--text-secondary)">Auto-categorized</p>
