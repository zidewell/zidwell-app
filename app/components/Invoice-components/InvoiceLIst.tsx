...
const downloadPdf = async (invoice: any) => {
  try {
        ...
        <div class="container">
          <div class="header">
            <div class="business-info">
              ${invoice.business_logo ? `<img src="${invoice.business_logo}" alt="${invoice.business_name}" class="logo">` : (base64Logo ? `<img src="${base64Logo}" alt="${invoice.business_name}" class="logo">` : "")}
              <h2>${invoice.business_name || "Business Name"}</h2>
              <p>${invoice.from_email || invoice.business_email || ""}</p>
          ...
          <div class="section">
            <div class="billing-info">
              <div class="billing-section">
                <h3>Bill To:</h3>
                <p><strong>${invoice.client_name || invoice.bill_to || "Client Information"}</strong></p>
                ${invoice.client_email ? `<p>📧 ${invoice.client_email}</p>` : ""}
                ${invoice.client_phone ? `<p>📞 ${invoice.client_phone}</p>` : ""}
              </div>
              <div class="billing-section">
                <h3>From:</h3>
                <p><strong>${invoice.from_name || invoice.business_name || ""}</strong></p>
                <p>📧 ${invoice.from_email || invoice.business_email || ""}</p>
              </div>
            </div>
          </div>
          ...
          ${invoice.message ? `
            <div class="section">
              <div class="message-box">
                <h3>Message from ${invoice.from_name || invoice.business_name}:</h3>
                <p>${invoice.message}</p>
              </div>
            </div>
          ` : ""}
          ...
          ` : paidAmount > 0 ? `
            <div class="section">
              <div class="payment-info">
                ...
                <p>Payment Progress: ${Math.round((paidAmount / totalAmount) * 100)}%</p>
              </div>
            </div>
          ` : ""}
              ...
              <tbody>
                ${invoiceItems.map((item: any) => `
                  <tr>
                    ...
                    <td>₦${Number(item.total_amount || item.total || (item.quantity || 0) * (item.unit_price || item.unitPrice || 0)).toLocaleString()}</td>
                  </tr>
                `).join("")}
              </tbody>
            </table>
          ...
          <div class="totals">
            ...
            ` : invoice.fee_option === "customer" && feeAmount > 0 ? `
              <div class="total-row" style="font-size: 12px; color: #666;">
          ...
          <div class="footer">
            ...
            <p>If you have any questions about this invoice, please contact ${invoice.from_email || invoice.business_email}</p>
    ...
    a.href = url;
    a.download = `invoice-${invoice.invoice_id}-${Date.now()}.pdf`;
