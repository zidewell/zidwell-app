      ...
      <div>
        {contractContent ? (
          <div className="space-y-8">
            {/* Document/PDF View */}
            <div className="bg-(--bg-primary) border border-(--border-color) rounded-lg shadow-lg overflow-hidden print:shadow-none print:border-none">
              {/* Document Header */}
              <div
                id="contract-document"
                className="p-6 min-h-[80vh] bg-(--bg-primary)"
              >
                {/* Contract Title */}
                ...
                {/* Parties Section */}
                <div className="mb-12">
                  <h3 className="text-lg font-semibold text-(--text-primary) mb-6 pb-2 border-b border-(--border-color)">
                  ...
                  <div className="grid md:grid-cols-2 gap-8">
                    <div className="space-y-4">
                      <div>
                        ...
                        <div className="bg-(--bg-secondary) p-4 rounded border border-(--border-color)">
                ...
                <div className="mb-12">
                  ...
                  <div className="max-w-none text-(--text-primary) leading-relaxed font-serif bg-(--bg-secondary) p-6 rounded border border-(--border-color)">
                    {contractContent ? (
                      <div
                ...
                {/* Signatures Section */}
                <div className="mt-16">
                  <h3 className="text-lg font-semibold text-(--text-primary) mb-8 pb-2 border-b border-(--border-color)">
                    SIGNATURES
                  </h3>
                  <div
                  ...
                  >
                    ...
                    <div className="space-y-4">
                      <div className="border-t border-(--border-color) pt-4">
                        <p className="text-sm font-medium text-(--text-secondary) mb-4 text-center">
                          PARTY A
                        </p>
                        <div className="h-32 border-b border-(--border-color) flex items-end justify-center mb-4">
                          {localSignature ? (
                            ...
                            <div className="h-20 w-40 border border-dashed border-(--border-color) rounded flex items-center justify-center">
                    ...
                    {/* Lawyer Signature (Conditionabg-(--color-accent-yellow)
                    {includeLawyerSignature && (
                      <div className="space-y-4">
                        <div className="border-t border-(--border-color) pt-4">
                          <div className="flex items-center mb-4">
                            <div className="h-6 w-6 bg-(--color-accent-yellow) rounded-full flex items-center justify-center mr-2">
                              <span className="text-(--color-ink) text-xs">
                                ✓
                              </span>
                            </div>
                            <p className="text-sm font-medium text-(--color-accent-yellow)">
                              LEGAL WITNESS SIGNATURE
                            </p>
                          </div>
                          <div className="h-32 border-b border-(--border-color) flex items-end justify-center mb-4">
                            <div className="text-center">
                              <p className="text-(--text-secondary) italic font-serif text-lg">
                                Barr. Adewale Johnson
                              </p>
                            </div>
                          </div>
                          <div className="text-cenbg-(--color-accent-yellow)
                            <p className="font-semibold text-(--text-primary)">
                              Barr. Adewale Johnson
                            </p>
                            <p className="text-sm text-(--text-secondary) mt-1">
            ...
            {attachments.length > 0 && (
              <div className="border border-(--border-color) rounded-lg p-6 bg-(--bg-secondary)">
                <h4 className="text-lg font-medium mb-4 text-(--text-primary) flex items-center">
                  <FileIcon className="h-5 w-5 mr-2 text-(--text-secondary)" />
                ...
                <div className="space-y-3">
                  {attachments.map((attachment, index) => (
                    <div
                      key={index}
                      className="flex items-center justify-between p-4 border border-(--border-color) rounded-lg bg-(--bg-primary) hover:bg-(--bg-secondary)"
                    >
            ...
            {/* Navigation Buttons */}
            <div className="flex flex-col sm:flex-row gap-3 pt-6 border-t border-(--border-color) print:hidden">
          ...
          <div className="text-center py-12">
            <div className="max-w-md mx-auto">
              <div className="h-16 w-16 bg-(--bg-secondary) rounded-full flex items-center justify-center mx-auto mb-4">
