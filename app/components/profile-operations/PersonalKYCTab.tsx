...
const PersonalKYCTab: React.FC = () => {
  ...
  const [userBankDetails, setUserBankDetails] = useState<UserWalletData | null>(null);
  ...
  const fetchUserBankDetails = async (forceRefresh = false) => {
    ...
    console.log(`🔄 Fetching bank details for user ${userData.id}, forceRefresh: ${forceRefresh}`);
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        // Log the personal bank details specifically
        ...
        // Update form data with fetched personal bank details
        setFormData(prev => {
          const newFormData = {
            ...prev,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (formData.ninNumber || formData.idCardFile || formData.utilityBillFile) {
        const kycFormData = new FormData();
        kycFormData.append("userId", userData.id);
        if (formData.ninNumber) kycFormData.append("nin", formData.ninNumber);
        if (formData.idCardFile) kycFormData.append("idCard", formData.idCardFile);
        if (formData.utilityBillFile) kycFormData.append("utilityBill", formData.utilityBillFile);
    ...
    <div className="space-y-6">
      <div className="neo-card bg-card p-6 space-y-5">
        ...
        {userBankDetails?.p_account_name && (
          <div className="bg-[#2b825b]/5 border-2 border-[#2b825b] rounded-md p-4 mb-4">
            <div className="flex items-center justify-between mb-2">
              <h4 className="font-heading text-foreground text-sm">YOUR PERSONAL BANK ACCOUNT</h4>
              <button
                onClick={() => fetchUserBankDetails(true)}
                disabled={fetchingBankDetails}
                className="text-[#2b825b] hover:text-[#2b825b]/80 text-xs flex items-center gap-1"
              >
                <RefreshCw className={`w-3 h-3 ${fetchingBankDetails ? 'animate-spin' : ''}`} />
                Refresh
              </button>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-sm">
              <div>
                ...
                <p className="font-medium text-foreground">{userBankDetails.p_bank_name || 'Not set'}</p>
              </div>
              <div>
                <span className="text-muted-foreground text-xs">Account Number:</span>
                <p className="font-medium text-foreground">{userBankDetails.p_account_number || 'Not set'}</p>
              </div>
              <div>
                <span className="text-muted-foreground text-xs">Account Name:</span>
                <p className="font-medium text-foreground">{userBankDetails.p_account_name || 'Not set'}</p>
              </div>
            </div>
          </div>
        )}
        ...
        {/* NIN Number */}
        <div>
          <label className="text-sm font-body text-muted-foreground block mb-1.5">NIN Number (Optional)</label>
          <input
            type="text"
            inputMode="numeric"
            maxLength={11}
            value={formData.ninNumber}
            onChange={(e) => handleChange("ninNumber", e.target.value.replace(/\D/g, ""))}
            className={inputClassName('ninNumber')}
            placeholder="Enter your 11-digit NIN"
            disabled={loading || fetchingBankDetails}
          />
          {errors.ninNumber && <p className="text-xs text-red-500 mt-1">{errors.ninNumber}</p>}
        </div>

        {/* Address */}
        <div>
          <label className="text-sm font-body text-muted-foreground block mb-1.5">Address</label>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <input
              type="text"
              value={formData.country}
              onChange={(e) => handleChange("country", e.target.value)}
              className="w-full bg-background border-2 border-[#2b825b] px-3 py-2 text-sm font-body text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-[#2b825b] focus:ring-2 focus:ring-[#2b825b]/20 transition-colors rounded-md disabled:opacity-50 disabled:cursor-not-allowed"
              placeholder="Country"
              disabled={loading || fetchingBankDetails}
            />
            <input
        ...
        <div className="border-t-2 border-[#2b825b] pt-4">
          <div className="flex items-center justify-between mb-3">
            <h3 className="font-heading text-foreground text-sm">PERSONAL BANK ACCOUNT DETAILS</h3>
            {fetchingBankDetails && (
              <span className="text-xs text-[#2b825b] flex items-center gap-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              <label className="text-sm font-body text-muted-foreground block mb-1.5">Bank Name</label>
              <select
                ...
                className="w-full bg-background border-2 border-[#2b825b] px-3 py-2 text-sm font-body text-foreground focus:outline-none focus:border-[#2b825b] focus:ring-2 focus:ring-[#2b825b]/20 transition-colors rounded-md disabled:opacity-50 disabled:cursor-not-allowed appearance-none"
                disabled={loading || fetchingBankDetails}
              >
              ...
              {/* Display current bank name for debugging */}
              {formData.bankName && (
                <p className="text-xs text-green-600 mt-1">Selected: {formData.bankName}</p>
              )}
            </div>
            <div>
              <label className="text-sm font-body text-muted-foreground block mb-1.5">Account Number</label>
              <input
                type="text"
                inputMode="numeric"
                maxLength={10}
          ...
          <div className="mt-3">
            <label className="text-sm font-body text-muted-foreground block mb-1.5">Account Name</label>
            <input
              type="text"
        ...
        {/* KYC Uploads */}
        <div className="border-t-2 border-[#2b825b] pt-4">
          <h3 className="font-heading text-foreground text-sm mb-3">KYC DOCUMENTS</h3>
          ...
          <div className="mb-4">
            ...
            <input
              ...
              onChange={(e) => handleFileChange("idCardFile", e.target.files?.[0] || null)}
              className="w-full bg-background border-2 border-[#2b825b] p-2 text-sm font-body text-foreground rounded-md file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-medium file:bg-[#2b825b] file:text-white hover:file:bg-[#2b825b]/90 disabled:opacity-50 disabled:cursor-not-allowed"
              disabled={loading || fetchingBankDetails}
            />
          </div>
          ...
          <div>
            ...
            <input
              type="file"
              accept="image/*,application/pdf"
              onChange={(e) => handleFileChange("utilityBillFile", e.target.files?.[0] || null)}
              className="w-full bg-background border-2 border-[#2b825b] p-2 text-sm font-body text-foreground rounded-md file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-medium file:bg-[#2b825b] file:text-white hover:file:bg-[#2b825b]/90 disabled:opacity-50 disabled:cursor-not-allowed"
              disabled={loading || fetchingBankDetails}
            />
          </div>
