...
const TestimonialsSection = () => {
  return (
    <section className=" bg-secondary/30 relative overflow-hidden py-20 md:py-28 lg:py-32 p-5">
      {/* Decorative elements */}
      <div className="absolute top-10 right-10 w-32 h-32 bg-linear-to-br from-(--color-accent-yellow)/20 to-transparent rounded-full blur-2xl" />
      <div className="absolute bottom-10 left-10 w-24 h-24 bg-linear-to-tr from-(--color-accent-yellow)/15 to-transparent rounded-full blur-xl" />
          ...
          {testimonials.map((testimonial, index) => (
            ...
            >
              ...
              <div className="absolute top-6 right-6 w-10 h-10 bg-(--color-accent-yellow)/20 flex items-center justify-center">
                <Quote className="w-5 h-5 text-(--color-accent-yellow)" />
              </div>
              ...
              <div className="flex gap-1 mb-4">
                {Array.from({ length: testimonial.rating }).map((_, i) => (
                  <Star
                    key={i}
                    className="w-4 h-4 fill-(--color-accent-yellow) text-(--color-accent-yellow)"
                  />
                ))}
              </div>
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-(--color-accent-yellow) font-semibold">
