...
const AudioPlayer = ({ content }: AudioPlayerProps) => {
  ...
  useEffect(() => {
    ...
    const processContent = async () => {
      try {
        ...
        wordsArrayRef.current = plainText
          .split(/\s+/)
  ...
  const togglePlay = () => {
    if (!isSupported) {
      alert(
        "Your browser doesn't support text-to-speech. Try using Chrome, Edge, or Safari.",
      );
      return;
    }
  ...
  const handleSliderChange = (value: number[]) => {
    ...
    const newWordIndex = Math.floor(
      (newProgress / 100) * totalWordsRef.current,
    );
  ...
  const getEstimatedTime = () => {
    ...
    const currentSeconds =
      (currentWordIndex / totalWordsRef.current) * estimatedSeconds;
    return {
