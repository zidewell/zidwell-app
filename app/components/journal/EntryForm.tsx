      ...
      <DialogContent className="sm:max-w-2xl md:max-w-3xl lg:max-w-4xl w-[95vw] max-h-[90vh] overflow-y-auto bg-(--bg-primary) border border-(--border-color) shadow-soft squircle-lg p-4 sm:p-6 md:p-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-5 md:space-y-6 mt-4">
          ...
          <div className="space-y-2">
            ...
            {/* Category list view - shows full names */}
            <div className="space-y-2 max-h-60 overflow-y-auto pr-1">
              {getSortedCategories.map((cat) => (
                <button
                  ...
                  className={cn(
                    "w-full flex items-center gap-3 px-3 py-2.5 md:px-4 md:py-3 rounded-xl border transition-all text-left relative hover:bg-(--bg-secondary) active:scale-[0.99] duration-200",
                ...
                >
                  {/* Icon */}
                  <span className="text-xl md:text-2xl shrink-0 w-8 text-center">
                    {cat.icon}
                  </span>

                  {/* Name - full width, no truncation */}
                  <span className="flex-1 text-sm md:text-base font-medium text-(--text-primary)">
                    {cat.name}
                  </span>

                  {/* Favorite star */}
                  {cat.isFavorite && (
                    <Star className="w-4 h-4 shrink-0 fill-(--color-accent-yellow) text-(--color-accent-yellow)" />
                  )}
