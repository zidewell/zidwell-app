...
const InvestmentFields = ({
  ...
  riskExplanation, setRiskExplanation,
}: Props) => (
  <div className="space-y-5">
    <div>
      <Label className="text-sm font-semibold mb-1.5 block">Minimum Investment Amount (₦) *</Label>
      <Input type="number" placeholder="e.g. 50000" value={minimumAmount} onChange={(e) => setMinimumAmount(e.target.value)} className="h-12" />
    </div>

    <div>
      <Label className="text-sm font-semibold mb-1.5 block">Expected Return (optional)</Label>
      <Input placeholder='e.g. "10-15% per annum" — clearly state this is an estimate' value={expectedReturn} onChange={(e) => setExpectedReturn(e.target.value)} className="h-12" />
    ...
    <div>
      ...
      <Input placeholder='e.g. "6 months", "1 year", "Flexible"' value={tenure} onChange={(e) => setTenure(e.target.value)} className="h-12" />
    </div>

    <div>
      <Label className="text-sm font-semibold mb-1.5 block">Fees / Charges</Label>
      <Input placeholder='e.g. "2% management fee", "₦500 processing fee"' value={charges} onChange={(e) => setCharges(e.target.value)} className="h-12" />
    </div>
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {(["one-time", "recurring"] as const).map((val) => (
          <button key={val} onClick={() => setPaymentFrequency(val)} className={`p-3 rounded-xl border-2 text-sm font-medium transition-all ${paymentFrequency === val ? "border-[#e1bf46] bg-[#e1bf46]/10 text-[#023528]" : "border-[#ded4c3] bg-[#f9f6ef] text-[#3e7465] hover:border-[#e1bf46]/50"}`}>
