    ...
    <div className="relative bg-(--bg-primary) border-2 border-(--border-color) rounded-md p-5 md:p-6 shadow-[4px_4px_0px_var(--border-color)]">
      <div className="flex items-center gap-5">
        ...
        <div className="flex-1 min-w-0">
          <p className="font-bold text-base text-(--text-primary)">{a.title}</p>
          <p className="text-sm text-(--text-secondary) mt-1 truncate font-['Be_Vietnam_Pro']">
            {a.text}
          </p>
        </div>
        <button className="hidden sm:inline-flex text-sm font-bold text-(--color-accent-yellow) hover:underline shrink-0 uppercase tracking-wide">
          {a.cta}
        </button>
      </div>
      ...
      <div className="flex justify-center gap-2 mt-4">
        {announcements.map((_, i) => (
          <button
            ...
            className={`w-2.5 h-2.5 rounded-full border-2 border-(--border-color) transition-colors ${
              i === current
