...
export async function POST(req: Request) {
  try {
    ...
    // 3️⃣ Get profile from `users` table
    const { data: userProfile } = await supabase
      .from("users")
      .select(
        "id, full_name, email, phone, wallet_balance, zidcoin_balance, referral_code, bvn_verification, admin_role, city, state, address, date_of_birth, profile_picture, current_login_session"
      )
      .eq("id", userId)
      .maybeSingle();
