...
import { processCardPaymentWebhook } from "./services/card-payment.service";
import { handleBank78Webhook } from "./bank78-handler";
...
// ============================================================
// HELPER: Detect which provider this webhook is for
// ============================================================
function detectPaymentProvider(payload: any): 'nomba' | 'bank78' | 'unknown' {
  const tx = payload.data?.transaction || {};
  const order = payload.data?.order || {};
  const eventType = payload.event_type || payload.eventType || '';
  
  // Check if it's a Bank78 webhook (Bank78 uses different event naming)
  if (payload.event || payload.event_type?.includes('bank78')) {
    return 'bank78';
  }
  
  // Check virtual account reference patterns
  const aliasAccountReference = tx.aliasAccountReference || tx.alias_account_reference || '';
  const orderReference = order.orderReference || '';
  
  // Bank78 virtual accounts typically have specific patterns
  // Check if the account reference looks like a Bank78 VA
  if (aliasAccountReference && 
      (aliasAccountReference.startsWith('B78-') || 
       aliasAccountReference.startsWith('BANK78-') ||
       aliasAccountReference.includes('BANK78'))) {
    return 'bank78';
  }
  
  // Check if order reference indicates Bank78
  if (orderReference && 
      (orderReference.startsWith('B78-') || 
       orderReference.startsWith('BANK78-'))) {
    return 'bank78';
  }
  
  // Check Bank78 signature header
  // If the request has Bank78 signature header, it's from Bank78
  // This will be checked in the webhook handler
  
  // Default to Nomba (backward compatibility)
  return 'nomba';
}

// ============================================================
// HELPER: Check if request is from Bank78 by headers
// ============================================================
function isBank78Request(req: NextRequest): boolean {
  const signature = req.headers.get('x-bank78-signature');
  const webhookType = req.headers.get('x-webhook-type');
  return !!(signature || webhookType === 'bank78');
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Check if this is a Bank78 webhook first (by headers)
    if (isBank78Request(req)) {
      console.log("🏦 Routing to Bank78 webhook handler");
      const rawBody = await req.text();
      let payload;
      try {
        payload = JSON.parse(rawBody);
      ...
      return await handleBank78Webhook(payload, req);
    }

    // If not Bank78, process as Nomba webhook
    console.log("💳 Routing to Nomba webhook handler");
    ...
    // Process Nomba webhook with existing logic
    return await processNombaWebhook(payload);
...
// ============================================================
// NOMBA WEBHOOK PROCESSING (Existing Logic)
// ============================================================
async function processNombaWebhook(payload: any): Promise<NextResponse> {
  const eventType = payload.event_type || payload.eventType;
  const tx = payload.data?.transaction || {};
  const order = payload.data?.order || {};
  const customer = payload.data?.customer || {};
  ...
  console.log("Processing Nomba webhook:", {
    eventType,
    amount: transactionAmount,
    aliasAccountReference,
    orderReference,
  ...
  if (isCardPayment && (eventType === "payment_success" || txStatus === "success")) {
    ...
    const result = await processCardPaymentWebhook(payload, {
      nombaTransactionId,
      orderReference,
      payment: await findPaymentByOrderReference(orderReference),
    });
    return handleErrorResponse(result);
  }

  // ============================================================
  // PRIORITY 1: PAYMENT PAGE VIRTUAL ACCOUNT
  // ============================================================
  ...
  if (isPaymentPageVA && (eventType === "payment_success" || txStatus === "success")) {
    ...
    const result = await processPaymentPageVirtualAccount(payload, {
      ...
      customer,
      tx,
      transferReference: extractTransferReference(tx),
    });
    return handleErrorResponse(result);
  }
  ...
  console.log("ℹ️ Unhandled Nomba event type:", eventType);
  return NextResponse.json({ message: "Event ignored" }, { status: 200 });
}
...
function checkIfCardPayment(orderReference: string, payload: any): boolean {
  if (orderReference?.startsWith("CARD-")) return true;
  const metadata = payload.data?.order?.metadata || {};
  if (metadata.type === "payment_page" && metadata.paymentMethod === "card") return true;
  const allowedMethods = payload.data?.order?.allowedPaymentMethods || [];
  if (allowedMethods.includes("Card") && !payload.data?.order?.virtualAccount) return true;
  return false;
}
...
function extractTransferReference(tx: any): string | null {
  const narration = tx.narration || tx.reference || "";
  const patterns = [/PPL[A-Za-z0-9]+/, /PPL_[A-Za-z0-9]+/, /PPL-[A-Za-z0-9]+/];
  for (const pattern of patterns) {
    const match = narration.match(pattern);
    if (match) return match[0];
  }
  if (narration && narration.length > 20) return narration;
  return null;
}
...
async function isRegularWalletDeposit(aliasAccountReference: string): Promise<boolean> {
  ...
  const { data: user } = await supabase.from("users").select("id").eq("id", aliasAccountReference).single();
  return !!user;
}

async function findPaymentByOrderReference(orderReference: string) {
  const { createClient } = await import("@supabase/supabase-js");
  const supabase = createClient(process.env.SUPABASE_URL!, process.env.SUPABASE_SERVICE_ROLE_KEY!);
