...
const AdminComments = () => {
  ...
  const fetchComments = async (page = 1) => {
    try {
      ...
      const response = await fetch(
        `/api/blog/comments?page=${page}&limit=${pagination.limit}&postId=all`,
      );
  ...
  const processCommentAction = async () => {
    ...
    try {
      ...
      toast.error(
        `Failed to ${actionType} comment: ${error instanceof Error ? error.message : "Unknown error"}`,
      );
    } finally {
  ...
  const filteredComments = comments.filter((comment) => {
    const matchesSearch =
      ...
      (comment.user_email || "")
        .toLowerCase()
        .includes(searchQuery.toLowerCase()) ||
      (comment.postTitle?.toLowerCase() || "").includes(
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-muted-foreground">
              Moderate and manage comments ({filteredComments.length} shown of{" "}
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            {!comment.is_approved ? (
                              <Button
                                ...
                                title="Approve"
                                onClick={() =>
                                  handleActionClick(comment, "approve")
                                }
                                disabled={
                                  isProcessing && processingId === comment.id
                                }
                              ...
                              <Button
                                variant="ghost"
                                size="icon"
                                className="text-yellow-600 hover:text-yellow-700 hover:bg-green-50"
                                title="Reject"
                                onClick={() =>
                                  handleActionClick(comment, "reject")
                                }
                                disabled={
                                  isProcessing && processingId === comment.id
                                }
                            ...
                            <Button
                              ...
                              title="Delete"
                              onClick={() =>
                                handleActionClick(comment, "delete")
                              }
                              disabled={
                                isProcessing && processingId === comment.id
                              }
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <Badge
                    variant={
                      selectedComment.is_approved ? "default" : "secondary"
                    }
                  >
                ...
                <div className="col-span-2">
                  ...
                  <p>
                    {format(new Date(selectedComment.created_at), "PPP 'at' p")}
                  </p>
                </div>
              </div>
              <div>
              ...
              <div className="flex justify-end gap-2 pt-4">
                {!selectedComment.is_approved ? (
                  <Button
                    ...
                    onClick={() => {
                      ...
                      setTimeout(
                        () => handleActionClick(selectedComment, "approve"),
                        100,
                      );
                    }}
                  >
                  ...
                  <Button
                    ...
                    onClick={() => {
                      setViewDialogOpen(false);
                      setTimeout(
                        () => handleActionClick(selectedComment, "reject"),
                        100,
                      );
                    }}
                  >
                ...
                <Button
                  ...
                  onClick={() => {
                    setViewDialogOpen(false);
                    setTimeout(
                      () => handleActionClick(selectedComment, "delete"),
                      100,
                    );
                  }}
                >
